...
type Until a i
  = String -> (a -> a -> Bool) -> a -> [Val i] -> Either (Error i) Bool

type Acc a i
  = String -> (a -> a -> a) -> a -> [Val i] -> Either (Error i) (Val i)
...
untilBool name cond = go
 where
  go _ [] = return True
  go x (Bool y _ : rest) | cond x y  = return False
                         | otherwise = go y rest
...
untilI8 name cond = go
 where
  go _ [] = return True
  go x (Int8 y _ : rest) | cond x y  = return False
                         | otherwise = go y rest
...
untilI16 name cond = go
 where
  go _ [] = return True
  go x (Int16 y _ : rest) | cond x y  = return False
                          | otherwise = go y rest
...
untilI32 name cond = go
 where
  go _ [] = return True
  go x (Int32 y _ : rest) | cond x y  = return False
                          | otherwise = go y rest
...
untilI64 name cond = go
 where
  go _ [] = return True
  go x (Int64 y _ : rest) | cond x y  = return False
                          | otherwise = go y rest
...
untilU8 name cond = go
 where
  go _ [] = return True
  go x (Uint8 y _ : rest) | cond x y  = return False
                          | otherwise = go y rest
...
untilU16 name cond = go
 where
  go _ [] = return True
  go x (Uint16 y _ : rest) | cond x y  = return False
                           | otherwise = go y rest
...
untilU32 name cond = go
 where
  go _ [] = return True
  go x (Uint32 y _ : rest) | cond x y  = return False
                           | otherwise = go y rest
...
untilU64 name cond = go
 where
  go _ [] = return True
  go x (Uint64 y _ : rest) | cond x y  = return False
                           | otherwise = go y rest
...
untilInt name cond = go
 where
  go _ [] = return True
  go x (Integer y _ : rest) | cond x y  = return False
                            | otherwise = go y rest
...
untilNat name cond = go
 where
  go _ [] = return True
  go x (Natural y _ : rest) | cond x y  = return False
                            | otherwise = go y rest
...
untilF16 name cond = go
 where
  go _ [] = return True
  go x (Float16 y _ : rest) | cond x y  = return False
                            | otherwise = go y rest
...
untilF32 name cond = go
 where
  go _ [] = return True
  go x (Float32 y _ : rest) | cond x y  = return False
                            | otherwise = go y rest
...
untilF64 name cond = go
 where
  go _ [] = return True
  go x (Float64 y _ : rest) | cond x y  = return False
                            | otherwise = go y rest
...
untilFlo name cond = go
 where
  go _ [] = return True
  go x (Flonum y _ : rest) | cond x y  = return False
                           | otherwise = go y rest
...
tail' [Text tx _] | Text.null tx = Left $ Message "empty" Nothing
...
builtin =
  ...
  where wrap (k, f) = (k, Builtin f Nothing)
