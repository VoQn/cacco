...
sub [v] =
    ($ Nothing) <$> case v of
        Int8 x _ -> pure $ Int8 $ negate x
        Int16 x _ -> pure $ Int16 $ negate x
        Int32 x _ -> pure $ Int32 $ negate x
        Int64 x _ -> pure $ Int64 $ negate x
        Integer x _ -> pure $ Integer $ negate x
        Natural x _ -> pure $ Integer $ negate $ fromIntegral x
        Float16 x _ -> pure $ Float16 $ negate x
        Float32 x _ -> pure $ Float32 $ negate x
        Float64 x _ -> pure $ Float64 $ negate x
        Flonum x _ -> pure $ Flonum $ negate x
        _ -> Left $ typeMismatch "-" "numeric" v
-- subtraction
sub (v : vs) = case v of
    Int8 x _ -> accI8 `startWith` x
