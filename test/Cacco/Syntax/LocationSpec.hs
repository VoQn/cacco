...
spec_instance_of_Pretty = do
    ...
    context "Location \"/home/cacco/foo.cacco\" 1 1 2 20" $ do
        ...
        it "(/home/cacco/foo.cacco:1,1-2,20)" $
            (show . pretty) x
                `shouldBe` "(/home/cacco/foo.cacco:1,1-2,20)"

spec_fromPositions :: Spec
spec_fromPositions = do
    it "(\"foo\":1,1) with (\"foo\":2,80)" $ do
        let a = P.initPosition & P.sourceName .~ "foo"
            b =
                P.initPosition & P.sourceName .~ "foo" & P.line .~ 2 & P.column .~ 80
    ...
    it "(\"foo\":1,1) with (\"bar\":1,1)" $ do
        let a = P.initPosition & P.sourceName .~ "foo"
            b =
                P.initPosition & P.sourceName .~ "bar" & P.line .~ 2 & P.column .~ 80
        evaluate (L.fromPositions a b)
        ...
        let
            ...
            x =
                L.initLocation
                    ...
                    & L.endColumn
                        .~ 80
            s = P.initPosition & P.sourceName .~ src
            e = P.initPosition & P.sourceName .~ src & P.line .~ 2 & P.column .~ 80
        toPositions x `shouldBe` (s, e)
