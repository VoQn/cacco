...
spec_Lexer_integer = do
  ...
  context "when parsing decimal integer literal" $ do
    it "can parse 0" $
      parseTest "0" `shouldBe` Right (Natural 0)

    it "can parse 1" $
      parseTest "1" `shouldBe` Right (Natural 1)

    it "can parse 10" $
      parseTest "10" `shouldBe` Right (Natural 10)

    it "can parse 010" $
      parseTest "010" `shouldBe` Right (Natural 10)
  ...
  context "when parsing hexadecimal integer literal" $ do
    it "can parse 0x0 as 0" $
      parseTest "0x0" `shouldBe` Right (Natural 0)
    --
    it "can parse 0x1 as 1" $
      parseTest "0x1" `shouldBe` Right (Natural 1)
    --
    it "can parse 0xa as 10" $
      parseTest "0xa" `shouldBe` Right (Natural 10)
    --
    it "can parse 0x10 as 16" $
      parseTest "0x10" `shouldBe` Right (Natural 16)
    --
    it "can parse 0x11 as 17" $
      parseTest "0x11" `shouldBe` Right (Natural 17)
    --
    it "can parse 0xFF as 255" $
      parseTest "0xFF" `shouldBe` Right (Natural 255)
    --
    it "can parse 0x100 as 256" $
      parseTest "0x100" `shouldBe` Right (Natural 256)
  --
  context "when parsing octal integer literal" $ do
    it "can parse 0o10 as 8" $
      parseTest "0o10" `shouldBe` Right (Natural 8)

    it "can parse 0o77 as integer" $
      parseTest "0o77" `shouldBe` Right (Natural 0o77)

  context "when parsing binary integer literal" $ do
    it "can parse 0b00000000 as 0" $
      parseTest "0b00000000" `shouldBe` Right (Natural 0)

    it "can parse 0b00000001 as 1" $
      parseTest "0b00000001" `shouldBe` Right (Natural 1)

    it "can parse 0b00000010 as 2" $
      parseTest "0b00000010" `shouldBe` Right (Natural 2)

    it "can parse 0b00000011 as 3" $
      parseTest "0b00000011" `shouldBe` Right (Natural 3)

    it "can parse 0b00000100 as 4" $
      parseTest "0b00000100" `shouldBe` Right (Natural 4)

    it "can parse 0b00000101 as 5" $
      parseTest "0b00000101" `shouldBe` Right (Natural 5)
