...
spec_declAst = do
  ...
  context "Definition, Assign" $ do
    it "(= x 10)" $ declParser "(= x 10)" `shouldBe` Right
    ...
    it "(= (const x _) x)" $ declParser "(= (const x _) x)" `shouldBe` Right
      (Def (App (Var PattProxy "const") [Var PattProxy "x", Hole PattProxy])
           (Var ExprProxy "x" :| [])
      )
    --
